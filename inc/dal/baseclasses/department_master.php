...
function select()
{
$query = "SELECT * FROM department_master WHERE ";
...
try
{
	$sql = $this->conn->prepare("SELECT * FROM department_master");
	//echo "<pre>";print_r($sql);
	$sql->setFetchMode(PDO::FETCH_ASSOC);
	$sql->execute();
...
function delete()
{
	try
	{	
		$sql =  $this->conn->prepare("DELETE FROM department_master  WHERE $this->condition");
		//echo "<pre>";print_r($sql);
		$result = $sql->execute();
		$this->numRows = $sql->rowCount();
		return $result;
...
$columnValue = implode(',',$valueClause);
	try
	{	
		$sql = $this->conn->prepare("INSERT INTO department_master ($columnName ) VALUES ( $tempColumnValues )");
...
$setClause = array();
	...
	try
	{	
		$sql = $this->conn->prepare("UPDATE department_master SET  $columnName WHERE $this->condition ");
		//echo "<pre>";print_r($sql); print_r($valueClause);
		$result = $sql->execute($valueClause);
