...
try
{
	$sql = $this->conn->prepare("SELECT * FROM state");
	//echo "<pre>";print_r($sql);
	$sql->setFetchMode(PDO::FETCH_ASSOC);
	$sql->execute();
...
function delete()
{
	try
	{	
		$sql =  $this->conn->prepare("DELETE FROM state  WHERE $this->condition");
		//echo "<pre>";print_r($sql);
		$result = $sql->execute();
		$this->numRows = $sql->rowCount();
		return $result;
...
$columnValue = implode(',',$valueClause);
	try
	{	
		$sql = $this->conn->prepare("INSERT INTO state ($columnName ) VALUES ( $tempColumnValues )");
...
$setClause = array();
	...
	try
	{	
		$sql = $this->conn->prepare("UPDATE state SET  $columnName WHERE $this->condition ");
		//echo "<pre>";print_r($sql); print_r($valueClause);
		$result = $sql->execute($valueClause);
