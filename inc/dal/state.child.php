...
require_once('baseclasses/state.php');

class stateChild extends state
{ 	// class : begin
...
$query = "SELECT $this->selectColumn FROM state WHERE ";
$condition= $this->condition;
$param = $this->param;
	if (is_array($condition)) 
	{
...
$this->id = "" ;
	try
	{
		...
		$sql = $this->conn->prepare("INSERT INTO state (" . implode(", ", $columnNames) . ") VALUES " . implode(", ", $rowsSQL));
